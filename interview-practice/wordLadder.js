// This originally took up to >4 ms to run all the tests.
// With optimizations, we're down to ~1ms, but it's still not fast enough :(
...
function wordLadder (beginWord, endWord, wordList) {
  ...
  function buildSeq (bw, ew, seq, wl, allSeq) {
    ...
    for (let i = 0; i < wl.length; i += 1) {
      if (!seq[wl[i]] && offByOne(wl[i], bw)) nextWords[wl[i]] = wl[i]
    }
    ...
    if (nextWords[ew]) {
      ...
      for (let nextWord in nextWords) {
        buildSeq(nextWord, ew, Object.assign({}, seq, { [nextWord]: nextWord }), wl, allSeq)
      }
    }
  ...
  const sequences = buildSeq(beginWord, endWord, { [beginWord]: [beginWord] }, wordList, [])

  return sequences
    .reduce((acc, val) => Math.min(acc, Object.keys(val).length), (Object.keys(sequences[0] || [])).length)
}
